...
subroutine thrustTorque(n, Np, Tp, r, precurve, presweep, precone, &
    Rhub, Rtip, precurveTip, presweepTip, T, Q, Bfw)
    ...
    ! out
    real(dp), intent(out) :: T, Q, Bfw

    ! local
    real(dp) :: ds
    real(dp), dimension(n+2) :: rfull, curvefull, sweepfull, Npfull, Tpfull
    real(dp), dimension(n+2) :: thrust, torque, flapwise, x_az, y_az, z_az, cone, s
    integer :: i
    ...
    do i = 1, n+1
        ...
        Bfw = Bfw + 0.5_dp*(flapwise(i) + flapwise(i+1))*ds
    end do
...
!   gradient     of useful results: q t bfw
!   with respect to varying inputs: tp precurvetip q r t rtip np
!                precurve presweep presweeptip rhub bfw precone
!   RW status of diff variables: tp:out precurvetip:out q:in-zero
!                r:out t:in-zero rtip:out np:out precurve:out presweep:out
!                presweeptip:out rhub:out bfw:in-zero precone:out
SUBROUTINE THRUSTTORQUE_BV(n, np, npb, tp, tpb, r, rb, precurve, &
& precurveb, presweep, presweepb, precone, preconeb, rhub, rhubb, rtip, &
& rtipb, precurvetip, precurvetipb, presweeptip, presweeptipb, tb&
& , qb, bfwb, nbdirs)
!   USE DIFFSIZES
!  Hint: nbdirsmax should be the maximum number of differentiation directions
  IMPLICIT NONE
...
! out
!   REAL(dp) :: t, q, bfw
  REAL(dp), DIMENSION(nbdirs) :: tb, qb, bfwb
! local
  REAL(dp) :: ds
...
& , npfullb, tpfullb
  REAL(dp), DIMENSION(n+2) :: thrust, torque, flapwise, x_az, y_az, z_az&
& , cone, s
  REAL(dp), DIMENSION(nbdirs, n+2) :: thrustb, torqueb, flapwiseb, &
& x_azb, z_azb, coneb, sb
  INTEGER :: i
  INTRINSIC KIND
  INTRINSIC COS
  INTEGER :: arg1
...
! integrate Thrust and Torque (trapezoidal)
  ...
  DO i=n+1,1,-1
    ds = s(i+1) - s(i)
    DO nd=1,nbdirs
      tempb(nd) = 0.5_dp*ds*bfwb(nd)
      flapwiseb(nd, i) = flapwiseb(nd, i) + tempb(nd)
      flapwiseb(nd, i+1) = flapwiseb(nd, i+1) + tempb(nd)
      dsb(nd) = 0.5_dp*(torque(i)+torque(i+1))*qb(nd) + 0.5_dp*(thrust(i&
&       )+thrust(i+1))*tb(nd) + 0.5_dp*(flapwise(i)+flapwise(i+1))*bfwb(&
&       nd)
      tempb0(nd) = 0.5_dp*ds*qb(nd)
      torqueb(nd, i) = torqueb(nd, i) + tempb0(nd)
      torqueb(nd, i+1) = torqueb(nd, i+1) + tempb0(nd)
      tempb1(nd) = 0.5_dp*ds*tb(nd)
      thrustb(nd, i) = thrustb(nd, i) + tempb1(nd)
      thrustb(nd, i+1) = thrustb(nd, i+1) + tempb1(nd)
  ...
  DO nd=1,nbdirs
    npfullb(nd, :) = 0.0
    rfullb(nd, :) = 0.0
    rfullb(nd, :) = npfull*flapwiseb(nd, :)
    npfullb(nd, :) = COS(cone)*thrustb(nd, :) + rfull*flapwiseb(nd, :)
    z_azb(nd, :) = 0.0
    tpfullb(nd, :) = 0.0
